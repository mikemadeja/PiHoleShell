...
Describe 'Get-PiHoleStatsRecentBlocked (Integration)' -Tag 'Integration' {
    BeforeAll {
        ...
        if (Test-Path $configPath) {
            ...
            # Generates some real query traffic (including known ad/tracker domains that get
            # blocked) so this function has something recent to report.
            & (Join-Path (Split-Path $PSScriptRoot -Parent) 'Initialize-PiHoleTestData.ps1') -DnsServer $PiHoleServer.Host
        }
    }

    It 'returns recently blocked domains as formatted objects' -Skip:(-not $script:ConfigAvailable) {
        $result = Get-PiHoleStatsRecentBlocked -PiHoleServer $script:PiHoleServer -Password $script:PiHoleToken -IgnoreSsl $script:PiHoleIgnoreSsl -MaxResult 5
    ...
    It 'returns the raw API response when RawOutput is set' -Skip:(-not $script:ConfigAvailable) {
        $result = Get-PiHoleStatsRecentBlocked -PiHoleServer $script:PiHoleServer -Password $script:PiHoleToken -IgnoreSsl $script:PiHoleIgnoreSsl -RawOutput $true
        $result | Format-List | Out-String | Write-Host

        $result.blocked | Should -Not -BeNullOrEmpty
    }

    It 'errors when given a bad password' -Skip:(-not $script:ConfigAvailable) {
        $result = Get-PiHoleStatsRecentBlocked -PiHoleServer $script:PiHoleServer -Password 'definitely-not-the-real-token' -IgnoreSsl $script:PiHoleIgnoreSsl -ErrorVariable errOut -ErrorAction SilentlyContinue
